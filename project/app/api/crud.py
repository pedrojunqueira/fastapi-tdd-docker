from app.models.pydantic import CurrentUserSchema, SummaryPayloadSchema, SummaryUpdatePayloadSchema
...
async def put(id: int, payload: SummaryUpdatePayloadSchema) -> dict | None:
    ...
    if summary:
        updated_summary = await TextSummary.filter(id=id).select_related("user").first().values()
        if updated_summary:
